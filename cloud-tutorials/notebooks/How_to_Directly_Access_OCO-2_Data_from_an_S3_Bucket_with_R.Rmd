...
Earthdata authentication is handled using the `earthdatalogin` package. Authentication for S3 credentials is handled using a development version of the package. This installation should only need to be executed *once*.
...
`earthdatalogin` authentication with username and password to obtain S3 credentials. This function expects a `.netrc` file to be present; to generate a `.netrc` file with R, consider using the [earthdatalogin::edl_netrc()](https://boettiger-lab.github.io/earthdatalogin/reference/edl_netrc.html) function.
